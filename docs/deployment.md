...
The English rollout was verified on 14 September 2026 at commit `737a9d1`. The public chat and external embed fixture rendered English copy. `/ready` reported migration `0010_english_runtime`, dataset `topmed-demo:3.0.0`, 30 embedded chunks, and the current `deepseek-flash` model. A production Gold-tier question returned an `ANSWERABLE`, `VERIFIED` response with citations from **Employer-sponsored plans** and **Family members and dependents**. This was a bounded smoke test, not the paid 100-case live answer evaluation.
