  ...
  async function run(actionName: string, success: string, confirm?: string) {
    ...
    await action.mutateAsync({
      ...
      errorNotification: (error) => ({
        message: "The gate could not be completed.",
    ...
    <>
      ...
      <Panel className="mb-5">
        ...
        {draft && canEdit ? (
          <div className="mt-5 flex flex-wrap gap-2 border-t border-slate-100 pt-4">
            <Button disabled={busy} onClick={() => void run("VALIDATE", "Validation completed.")} type="button" variant="outline">
              <CheckCircle2 size={16} /> Validate
            </Button>
            <Button disabled={busy} onClick={() => void run("INDEX", "Indexing completed.")} type="button" variant="outline">
              <DatabaseZap size={16} /> Indexar
            </Button>
            <Button disabled={busy} onClick={() => void run("EVALUATE", "Evaluation completed.")} type="button" variant="outline">
              <FlaskConical size={16} /> Evaluate retrieval
            </Button>
            {canPublish ? (
              <Button
                disabled={busy}
                onClick={() => void run("ACTIVATE", "Version published.", "Publish this version and retire the currently active knowledge base?")}
                type="button"
              >
                Publish
              </Button>
        ...
        {version.status === "RETIRED" && canPublish ? (
          <div className="mt-5 border-t border-slate-100 pt-4">
            <Button
              disabled={busy}
              onClick={() => void run("ROLLBACK", "Rollback completed.", "Reactivate this version and retire the currently active knowledge base?")}
              type="button"
              variant="outline"
            >
              <RotateCcw size={16} /> Reactivate version
            </Button>
          </div>
        ) : null}
      </Panel>
      ...
      {version.validation && !version.validation.passed ? (
        <Panel className="mb-5 border-red-200 bg-red-50">
          <h2 className="font-semibold text-red-900">Validation errors</h2>
      ...
      {version.evaluation ? (
        <Panel className="mb-5">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <h2 className="font-semibold">Latest publication evaluation</h2>
          ...
          <details className="mt-3 rounded-xl border border-slate-200">
            <summary className="cursor-pointer px-4 py-3 text-sm font-medium">Verifiable metrics</summary>
            ...
            <Panel className="h-full transition hover:border-teal-300 hover:shadow-md">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-3 text-sm text-teal-700">{document.chunk_count} chunks · open review</p>
                </div>
              </div>
            </Panel>
          </Link>
